(() => {
    ...
    da.init();
    (function() {
        const allWraps = () => document.querySelectorAll("[data-video-wrap]");
        function resetOthers(excludeWrap) {
            allWraps().forEach(wrap => {
                if (wrap === excludeWrap) return;
                const vid = wrap.querySelector("video");
                const btn = wrap.querySelector(".slide-comments__play-btn");
                vid.pause();
                vid.currentTime = 0;
        ...
        allWraps().forEach(wrap => {
            ...
            const playBtn = wrap.querySelector(".slide-comments__play-btn");
    ...
    document.querySelectorAll(".js-more-btn").forEach(btn => {
        btn.addEventListener("click", function(e) {
            e.preventDefault();
            const card = this.closest(".slide-results");
            card.querySelectorAll(".slide-results__hidden").forEach(el => {
    ...
    if (path.includes("thanks")) fbq("track", "Lead");
    if (path.includes("order")) fbq("track", "Purchase");
    document.addEventListener("DOMContentLoaded", () => {
        ...
        function initPhoneMask(input) {
            input.addEventListener("input", () => {
                ...
                if (val.startsWith("380")) val = val.slice(0, 12); else if (val.startsWith("38")) val = val.slice(0, 12); else if (val.startsWith("0")) {
                    val = "38" + val;
                    val = val.slice(0, 12);
                } else if (val === "") {
                    input.value = "";
                ...
                let masked = "+38";
                if (val.length > 2) masked += " (" + val.slice(2, 5);
            ...
            input.addEventListener("keydown", e => {
                if (e.key === "Backspace" && input.value === "+38") input.value = "";
            });
        ...
        function validateField(input) {
            ...
            let error = "";
            if (input.name === "name") if (!value) error = "Введіть ім'я"; else if (value.length < 2) error = "Мінімум 2 символи";
            if (input.name === "phone") {
                const digits = value.replace(/\D/g, "");
                if (!value) error = "Введіть номер телефону"; else if (digits.length < 12) error = "Введіть повний номер";
            }
            if (input.name === "descr") if (!value) error = "Розкажіть про проєкт";
            showFieldError(input, error);
            return !error;
        }
        function showFieldError(input, message) {
            const item = input.closest(".item-form");
            let errorEl = item.querySelector(".item-form__error");
            if (!errorEl) {
                errorEl = document.createElement("span");
                errorEl.classList.add("item-form__error");
        ...
        function initForm(form, getTitle) {
            ...
            form.addEventListener("submit", async e => {
                ...
                try {
                    await fetch("https://hook.eu1.make.com/qb3yhgzwa139gf66end3qkr2i5gv7lrw", {
                        method: "POST",
                        headers: {
        ...
        document.querySelectorAll(".popup").forEach(popup => {
            const form = popup.querySelector(".main-form");
            if (!form) return;
            initForm(form, () => popup.querySelector(".popup__title")?.innerText.trim().replace(/\n/g, " ") || "");
        });
